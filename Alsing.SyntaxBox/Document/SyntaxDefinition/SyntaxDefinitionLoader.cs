...
namespace Alsing.Windows.Forms.Document.SyntaxDefinition
{
    ...
    public class SyntaxDefinitionLoader
    {
                ...
                if (n.NodeType == XmlNodeType.Element)
                {
                    if (n.Name.ToLowerInvariant() == "scope")
                    {
                        ...
                        if (Start != "")
                        {
                            ...
                            var scope = new Scope.Scope { Style = GetStyle(style), ExpansionText = text, DefaultExpanded = expanded, CauseIndent = blnCauseIndent};
                        ...
                        if (cn.Name.ToLowerInvariant() == "patterngroup")
                        {
                            ...
                            foreach (XmlNode pt in cn.ChildNodes)
                            {
                                if (pt.Name.ToLowerInvariant() == "pattern")
                                {
                                    ...
                                    if (Pattern != "")
                                    {
                                        var pat = new Pattern.Pattern(Pattern, IsComplex, IsSeparator, true) {Category = Category};
                                        pl.Add(pat);
                                    }
                                }
                        ...
                        if (cn.Name.ToLowerInvariant() == "patterngroup")
                        {
                            ...
                            foreach (XmlNode pt in cn.ChildNodes)
                            {
                                if (pt.Name.ToLowerInvariant() == "pattern")
                                {
                                    ...
                                    if (Pattern != "")
                                    {
                                        var pat = new Pattern.Pattern(Pattern, IsComplex, IsSeparator, false) {Category = Category};
                                        pl.Add(pat);
                                    }
                                }
                                else if (pt.Name.ToLowerInvariant() == "patterns")
        ...
        //done
        private SpanDefinition.SpanDefinition GetBlock(string Name)
        {
            if (spanDefinitionLookup[Name] == null)
            {
                var b = new SpanDefinition.SpanDefinition(syntaxDefinition);
                spanDefinitionLookup.Add(Name, b);
            }

            return (SpanDefinition.SpanDefinition) spanDefinitionLookup[Name];
        }
