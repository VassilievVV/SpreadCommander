...
namespace Alsing.Windows.Forms.Classes
{
    /// <summary>
    ...
    public class NativePainter : IPainter, IDisposable
    {
        ...
        public void InitGraphics()
        {
            ...
            if (Control != null)
            {
                if (Control.IsHandleCreated)
                {
                    ...
                    int h = GFX.StringBuffer.MeasureTabbedString("ABC", 0).Height + Control.SyntaxBox.RowPadding;
                    ...
                    Control.View.RowHeight = GFX.BackBuffer.MeasureTabbedString("ABC", 0).Height + Control.SyntaxBox.RowPadding;
            ...
            try
            {
                ...
                try
                {
                    if (RowIndex < Control.Document.Count && RowIndex >= 0)
                    {
                        ...
                        if (SpanFound && RowIndex >= FirstSpanRow && RowIndex <= LastSpanRow && Control.SyntaxBox.ScopeBackColor != Color.Transparent)
                        {
                            bg = new GDIBrush(Control.SyntaxBox.ScopeBackColor);
                            found = true;
                        }
        ...
        private void RenderText(int RowIndex, int XOffset, Word StartWord)
        {
            ...
            if (RowIndex <= Control.Document.Count - 1)
            {
                ...
                if (Control.SyntaxBox.ShowEOLMarker && !HasExpansion)
                {
                    bbuff.Font = GFX.FontNormal;
                    bbuff.TextForeColor = Control.SyntaxBox.EOLMarkerColor;
                    bbuff.FontTransparent = true;
                    bbuff.DrawTabbedString("¶", xpos, 0, taborig, Control.PixelTabSize);
                }
            }
        ...
        private void RenderExpansion(int RowIndex)
        {
            ...
            if (RowIndex <= Control.Document.Count - 1)
            {
                ...
                //RENDER SPAN MARGIN
                if (SpanFound && Control.SyntaxBox.ScopeIndicatorColor != Color.Transparent && Control.SyntaxBox.ShowScopeIndicator)
                {
                    if (RowIndex >= FirstSpanRow && RowIndex <= LastSpanRow)
                        bbuff.FillRect(Control.SyntaxBox.ScopeIndicatorColor, Control.View.TotalMarginWidth + 14, 0, 2, Control.View.RowHeight);

                    if (RowIndex == FirstSpanRow)
                        bbuff.FillRect(Control.SyntaxBox.ScopeIndicatorColor, Control.View.TotalMarginWidth + 14, 0, 4, 2);

                    if (RowIndex == LastSpanRow)
                        bbuff.FillRect(Control.SyntaxBox.ScopeIndicatorColor, Control.View.TotalMarginWidth + 14, Control.View.RowHeight - 2, 4, 2);
                }
            }
        }
