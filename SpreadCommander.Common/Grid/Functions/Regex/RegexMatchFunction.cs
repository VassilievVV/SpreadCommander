...
{
    public class RegexMatchFunction : ICustomFunctionOperatorBrowsable
    {
        public const string FunctionName = "RegexMatch";
        static readonly RegexMatchFunction instance = new RegexMatchFunction();
        ...
        public string Description                                                     => "Searches the specified input string for the first occurrence of the regular expression.\r\nParameters: input, pattern, match number.";
        public bool IsValidOperandCount(int count)                                    => (Utils.IsValueInRange(count, 2, 3));
        public bool IsValidOperandType(int operandIndex, int operandCount, Type type) => (Utils.IsValueInRange(operandCount, 2, 3));
        ...
        {
            if (operands == null || operands.Length < 2 || operands.Length > 3)
